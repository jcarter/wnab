...
const authorization = (token) => ['Bearer', token].join(' ');


describe('createYnabClient', () => {
  test('getPlans sends a read-only token GET request and unwraps data', async () => {
    const fetcher = vi.fn().mockResolvedValue(jsonResponse(PLANS_RESPONSE));
    const client = createYnabClient({ token: ' fake-token ', fetcher });
    ...
    expect(fetcher).toHaveBeenCalledExactlyOnceWith(`${YNAB_API_BASE_URL}/plans`, {
      method: 'GET',
      headers: {
        Authorization: authorization('fake-token'),
      },
    });
  });
  ...
  test('getPlanMonths URL-encodes the plan id and unwraps months data', async () => {
    ...
    const client = createYnabClient({ token: 'fake-token', fetcher, baseUrl: 'https://example.test/api' });
  ...
  test('throws before fetching when the access token is blank', async () => {
    const fetcher = vi.fn();
    const client = createYnabClient({ token: '   ', fetcher });

    await expect(client.getPlans()).rejects.toThrow('Missing YNAB access token');
